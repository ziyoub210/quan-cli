...
class InitAction {
  ...
  async getTemplateVersion() {
    return await utils_1.NpmUtil.getLatestVersion(this.origin + '/' + this.templates[0].store);
  }
  async install() {
    const version = await this.getTemplateVersion();
    utils_1.log.debug(`执行安装:${this.templates[0].store}，版本:${version}`);
    console.log(path_1.default.resolve(this.basePath, config_1.paths.ROOT_PATH, config_1.paths.TARGET_PATH));
    await (0, npminstall_1.default)({
      root: path_1.default.resolve(this.basePath, config_1.paths.ROOT_PATH, config_1.paths.TARGET_PATH),
